##### Monthly run #########
  ...
  dt <- 1
  days<-DaysInAMonth[month_index]
  times <- seq(t_start,(t_start+days-1),dt)
  k<-floor(m/12)+1 # the year index
  ...
  parameters<-set_parameters_model2pt1(parameters_demog, parameters_virus, zone, m)
 ...
 #times<-seq(t_start, 5, dt)
  ...
  sol <- as.data.frame(ode(init_allStates, times, CyHV3_5AgeClasses, parameters, method="lsoda"))
