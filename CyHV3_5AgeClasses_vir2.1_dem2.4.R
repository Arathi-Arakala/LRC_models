...
CyHV3_5AgeClasses_vir2pt1_dem2pt4<-function(t, State, Pars){
  ...
  with(as.list(c(State, Pars)), {
    ...
    for(i in 1:5){
      ...
      if(i==1){
        ...
        dS[i]= births -S[i]*(beta_mx[,i]%*%(I+ Z + A  ) ) -mu_mx[i]*S[i] - c_mx[i]*(S[i]*sum(Total))
        dE[i]=S[i]*(beta_mx[,i]%*%(I+ Z + A  ) )- eta*E[i] -mu_mx[i]*E[i] - c_mx[i]*(E[i]*sum(Total))
      ...
      if(i>2){
        dS[i]=-S[i]*(beta_mx[,i]%*%(I+ Z + A  ) ) -mu_mx[i]*S[i]
        dE[i]=S[i]*(beta_mx[,i]%*%(I+ Z + A  ) )- eta*E[i] -mu_mx[i]*E[i]
