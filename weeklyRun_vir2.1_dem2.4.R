...
for(w in 1:w_stop_index){
  ...
  if(data_zone$week_no[w]!=26){
    ...
    if( endOfYearFlag==TRUE){
      ...
      if(z==2)
        plot_demography(output_all_zone2, z, wetland_allZones[z,])
      plotDiseaseClasses(output_all_zone2, z, wetland_allZones[z,])
...
# In dem 2.4, we start we start in the year 2000, July 1st. This will be week 26.
#for(w in 1:runtime ){
for(w in (w_stop_index+1):runtime){
  year <-data_new$iso_year[w]
